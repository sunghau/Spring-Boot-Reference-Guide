...
如果你需要創建一個分層的ApplicationContext（多個具有父子關係的上下文），或你只是喜歡使用流暢的建構API，你可以使用SpringApplicationBuilder。SpringApplicationBuilder允許你以鏈式方式調用多個方法，包括可以創建層次結構的parent和child方法。
```java
new SpringApplicationBuilder()
    ...
    .run(args);
```
**注**：創建ApplicationContext層次時有些限製，比如，Web組件(components)必須包含在子上下文(child context)中，且相同的Environment即用於父上下文也用於子上下文中。具體參考[SpringApplicationBuilder javadoc](http://docs.spring.io/spring-boot/docs/1.2.2.BUILD-SNAPSHOT/api/org/springframework/boot/builder/SpringApplicationBuilder.html)
...
```
**注**：你也能使用相應的技巧為存在的Spring Boot屬性創建'短'變量，具體參考[Section 63.3, “Use ‘short’ command line arguments”](http://docs.spring.io/spring-boot/docs/current-SNAPSHOT/reference/htmlsingle/#howto-use-short-command-line-arguments)。
...
**注**：spring-boot-actuator模塊包含一個暴露所有@ConfigurationProperties beans的端點。簡單地將你的web瀏覽器指向/configprops或使用等效的JMX端點。具體參考[Production ready features](http://docs.spring.io/spring-boot/docs/current-SNAPSHOT/reference/htmlsingle/#production-ready-endpoints)。
...
```
**注**：你可以使用@EntityScan注解自定義實體掃描路徑。具體參考[Section 67.4, “Separate @Entity definitions from Spring configuration”](http://docs.spring.io/spring-boot/docs/current-SNAPSHOT/reference/htmlsingle/#howto-separate-entity-definitions-from-spring-configuration)。
...
Spring Boot會檢查你發布的jar中是否存在META-INF/spring.factories文件。該文件應該列出以EnableAutoConfiguration為key的配置類：
```java
org.springframework.boot.autoconfigure.EnableAutoConfiguration=\
com.mycorp.libx.autoconfigure.LibXAutoConfiguration,\
com.mycorp.libx.autoconfigure.LibXWebAutoConfiguration
```
如果配置需要應用特定的順序，你可以使用[@AutoConfigureAfter](http://github.com/spring-projects/spring-boot/tree/master/spring-boot-autoconfigure/src/main/java/org/springframework/boot/autoconfigure/AutoConfigureAfter.java)或[@AutoConfigureBefore](http://github.com/spring-projects/spring-boot/tree/master/spring-boot-autoconfigure/src/main/java/org/springframework/boot/autoconfigure/AutoConfigureBefore.java)注解。例如，你想提供web-specific配置，你的類就需要應用在WebMvcAutoConfiguration後麵。
